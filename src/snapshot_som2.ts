...
type Snapshot = GenericSnapshot<Contract.SOM2>;
...
export async function snapshotSom2(web3: Web3, blockNumber: number, bearingSnapshot: Snapshot): Promise<Snapshot> {
  ...
  await batchIterate(bearingBlockNumber + 1, blockNumber, +BATCH_SIZE, async (fromBlock, toBlock) => {
    ...
    const logs = await web3.eth.getPastLogs({
      address: CONTRACTS_ADDRESSES.SOM2,
      fromBlock,
      toBlock,
