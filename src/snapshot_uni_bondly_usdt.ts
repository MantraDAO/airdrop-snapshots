...
type Snapshot = GenericSnapshot<Contract.UNI_BONDLY_USDT>;

export async function snapshotUniBondlyUsdt(web3: Web3, blockNumber: number, bearingSnapshot: Snapshot): Promise<Snapshot> {
  ...
  const logs = await web3.eth.getPastLogs({
    address: config.CONTRACTS_ADDRESSES.UNI_BONDLY_USDT,
