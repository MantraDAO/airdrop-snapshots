    ...
    cleanedStakes.forEach(function(element) {
        if (cleanedUnstakes.find(e => e.address === element.address)) {
            snapshot.push({
                ...
                stakedBalances: (BigInt(element.stakedAmount) - BigInt(cleanedUnstakes.find(e => e.address === element.address).unstakedAmount))
            });
        } else {
            snapshot.push({
                address: element.address,
                stakedBalances: BigInt(element.stakedAmount)
            });
        }
    });

    snapshot.forEach((snap) => {
        if (snap.stakedBalance <= 0) {
            snap.stakedBalance = 0;
        }
    });
