...
type Snapshot = GenericSnapshot<Contract.SBONDLY>;
...
export async function snapshotSbondly(web3: Web3, blockNumber: number, bearingSnapshot: Snapshot): Promise<Snapshot> {
  ...
  await batchIterate(bearingBlockNumber + 1, blockNumber, +BATCH_SIZE, async (fromBlock, toBlock) => {
    ...
    const logs = await web3.eth.getPastLogs({
      address: CONTRACTS_ADDRESSES.SBONDLY,
      fromBlock,
      toBlock,
